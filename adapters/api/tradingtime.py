...
BEFORE_END_OFFSET = timedelta(seconds=45)
AFTER_START_OFFSET = timedelta(seconds=15)
MOSCOW_ZONE = zoneinfo.ZoneInfo('Europe/Moscow')


class TradingTime:
    def __init__(self) -> None:
        schedule = FutureTradingHours()
    ...
    def _update_trading_schedule_with_time_offset(self) -> None:
        ...
        self._trading_sessions = {
            title: TradingSession(
                open=(
                    datetime.combine(dt, period.open) + AFTER_START_OFFSET
                ).time(),
                close=(
                    datetime.combine(dt, period.close) - BEFORE_END_OFFSET
                ).time(),
            )
